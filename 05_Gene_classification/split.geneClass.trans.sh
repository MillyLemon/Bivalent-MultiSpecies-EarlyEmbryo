...
# 参考BED文件
REF_BED="geneWithDiff_trans.bed"
...
for sample_dir in `cat sample.name`; do
    ...
    if [[ -f "$result_file" ]]; then
        ...
        # 为当前样本创建三个BED文件
        broad_bed="${sample_dir}/broad_genes.trans.bed"
        promoter_bed="${sample_dir}/promoter_genes.trans.bed" 
        tss_bed="${sample_dir}/tss_genes.trans.bed"
